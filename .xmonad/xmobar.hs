Config {
         ...
         position = TopW L 90,
                    commands = [ Run MultiCpu ["-t CPUs: <total0>% <total1>%", "-p", "3", "-L","3","-H","50","--normal","grey","--high","red"] 10,
                                 ...
                                 Run Com "~/.xmonad/inet.sh" [] "inet" 50,
                    ...
                    sepChar = "%",
                    alignSep = "}{",
                    template = "%StdinReader% }{ %multicpu% <fc=blue>|</fc> %memory% <fc=blue>|</fc> %battery% <fc=blue>|</fc> Therms: %thermal0% %thermal1% %thermal2% <fc=blue>|</fc> Vol: %volume% <fc=blue>|</fc> Inet: %inet% <fc=blue>|</fc> %date%"
       }
